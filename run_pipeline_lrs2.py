...
class Args:
  def __init__(self, videofile, data_dir):
    ...
    # self.syncnet_videofile = os.path.join(self.data_dir, "pyavi", "temp_avi", "video.avi")
    self.reference = ""
    # self.reference = "temp_avi"
...
def main(args):
  ...
  opt = Args(video_path, os.path.join("processing_lrs2", output_path))
  ...
  # print("----->", os.path.join(opt.crop_dir,opt.reference))
  flist.sort()
  ...
  if not flist:
      with open(opt.logs, "a") as f:
          f.write(f"{opt.videofile},-----,-----,-----,{time.time() - st:.3f} s\n")
  for idx, fname in enumerate(flist):
      offset, conf, dist, offset, minval, conf = s.evaluate(opt,videofile=fname)
      with open(opt.logs, "a") as f:
          f.write(f"{opt.videofile},{offset},{minval:.3f},{conf:.3f},{time.time() - st:.3f} s\n")
      dists.append(dist)
...
if __name__ == "__main__":
    ...
    videos = [
        str(path)
        for path in Path(opt.data_root).rglob("*.mp4")
        if not path.name.startswith(".")][:64]
    
    if not os.path.exists(opt.logs):
        with open(opt.logs, "w") as f:
            f.write("video,AV_offset,Min_dist,Confidence,Elapsed_time\n")
    logs = pd.read_csv(opt.logs)
    # intervals = [4, 25, 44]
    
    print("Total video count:", len(videos))
    videos = sorted(list(set(videos) - set(logs["video"])))
    print("Processing videos:", len(videos))
    print("-" * 100)

    # for video in videos:
    #   splitted_path = video.split("/")
    #   csv_path = os.path.join("/", *splitted_path[:-1], splitted_path[-2]+".csv")
    #   df = pd.read_csv(csv_path)
    #   time_steps = [df["ElapsedTime"].iloc()[i].item() for i, j in enumerate(df["Tag"].iloc()) if j == "OK"]
    #   start_times = [int((time_steps[i+1] + time_steps[i]) // 2) for i in intervals]
    #   videos_to_process.extend([
    #     (video, i, start_time) for i, start_time in enumerate(start_times)
    #   ])
    ...
    with Pool(num_workers) as pool:
        pool.map(main, enumerate(videos))
